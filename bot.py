...
class CloseButton(discord.ui.View)
    ...
    @discord.ui.button(label=🗑️ 티켓 닫기, style=discord.ButtonStyle.red, emoji=🗑️)
...
# 티켓 패널 생성 함수
async def create_ticket_panel(ctx, panel_title, options dict, category, embed_color=0xFFD1DC, author_icon=httpsi.imgur.com6rJX5KT.png)
    class TicketDropdown(discord.ui.Select)
        def __init__(self)
            select_options = [
                discord.SelectOption(label=label, value=label, emoji=label[0])
                for label in options
            ]
            super().__init__(placeholder=✨ 원하는 항목을 선택해주세요!, options=select_options, min_values=1, max_values=1)
        ...
        async def callback(self, interaction discord.Interaction)
            ...
            await interaction.response.send_message(f티켓이 생성되었습니다 {ticket_channel.mention}, ephemeral=True)
